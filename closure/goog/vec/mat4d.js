...
/** @typedef {!googVec.Float64} */ export var Type;


/**
 * Creates a mat4d with all elements initialized to zero.
 *
 * @return {!Type} The new mat4d.
 */
 ...
 *     chained together.
 */
export function setFromMat4d(mat, src) {
  mat[0] = src[0];
  mat[1] = src[1];
  mat[2] = src[2];
  mat[3] = src[3];
 ...
 *     chained together.
 */
export function setFromMat4f(mat, src) {
  mat[0] = src[0];
  mat[1] = src[1];
  mat[2] = src[2];
  mat[3] = src[3];
 ...
 *
 * @param {!Type} mat The matrix to receive the values.
 * @param {!vec4d.Type} vec The vector containing the values.
 * @return {!Type} return mat so that operations can be
 *     chained together.
 */
 ...
 *
 * @param {!Type} mat The matrix containing the values.
 * @param {!vec4d.Type} vec The vector to receive the values.
 ...
 * @return {!vec4d.Type} return vec so that operations can be
 *     chained together.
 */
 ...
 * @param {!Type} mat The matrix to receive the values.
 * @param {number} column The column index to set the values on.
 * @param {!vec4d.Type} vec The vector of elements for the column.
 * @return {!Type} return mat so that operations can be
 *     chained together.
 */
 ...
 * @param {!vec4d.Type} vec The vector of elements to
 *     receive the column.
 * @return {!vec4d.Type} return vec so that operations can be
 *     chained together.
 */
 ...
 *
 * @param {!Type} mat The matrix to receive the values.
 * @param {!vec4d.Type} vec0 The values for column 0.
 * @param {!vec4d.Type} vec1 The values for column 1.
 * @param {!vec4d.Type} vec2 The values for column 2.
 * @param {!vec4d.Type} vec3 The values for column 3.
 * @return {!Type} return mat so that operations can be
 *     chained together.
 */
 ...
 * @param {!vec4d.Type} vec0 The vector to receive column 0.
 * @param {!vec4d.Type} vec1 The vector to receive column 1.
 * @param {!vec4d.Type} vec2 The vector to receive column 2.
 * @param {!vec4d.Type} vec3 The vector to receive column 3.
 */
 ...
 * @param {!vec4d.Type} vec The vector to receive the row.
 * @return {!vec4d.Type} return vec so that operations can be
 *     chained together.
 */
 ...
 *
 * @param {!Type} mat The matrix to receive the values.
 * @param {!vec4d.Type} vec0 The values for row 0.
 * @param {!vec4d.Type} vec1 The values for row 1.
 * @param {!vec4d.Type} vec2 The values for row 2.
 * @param {!vec4d.Type} vec3 The values for row 3.
 * @return {!Type} return mat so that operations can be
 *     chained together.
 */
 ...
 *
 * @param {!Type} mat The matrix to supply the values.
 * @param {!vec4d.Type} vec0 The vector to receive row 0.
 * @param {!vec4d.Type} vec1 The vector to receive row 1.
 * @param {!vec4d.Type} vec2 The vector to receive row 2.
 * @param {!vec4d.Type} vec3 The vector to receive row 3.
 */
 ...
 *
 * @param {!Type} mat The matrix to compute the matrix for.
 ...
 *
 * @param {!Type} mat The matrix to invert.
 * @param {!Type} resultMat The matrix to receive
 ...
 * @param {!vec3d.Type} vec The 3 element vector to transform.
 * @param {!vec3d.Type} resultVec The 3 element vector to
 *     receive the results (may be vec).
 * @return {!vec3d.Type} return resultVec so that operations can be
 *     chained together.
 */
 ...
 * @param {!Type} mat The matrix supplying the transformation.
 * @param {!vec3d.Type} vec The 3 element vector to transform.
 * @param {!vec3d.Type} resultVec The 3 element vector
 ...
 *
 * @param {!Type} mat The matrix supplying the transformation.
 * @param {!vec4d.Type} vec The vector to transform.
 * @param {!vec4d.Type} resultVec The vector to
 *     receive the results (may be vec).
 * @return {!vec4d.Type} return resultVec so that operations can be
 *     chained together.
 */
 ...
 *
 * @param {!Type} mat The matrix.
 * @param {!Quaternion.AnyType} rotation The quaternion rotation.
 *     Note: this quaternion is assumed to already be normalized.
 * @param {!vec3d.Type} translation The vector translation.
 * @return {!Type} return mat so that operations can be
 *     chained.
 */
export function makeRotationTranslation(mat, rotation, translation) {
  // Quaternion math
  const w = rotation[3];
  const x = rotation[0];
  const y = rotation[1];
 ...
 *     Note: this quaternion is assumed to already be normalized.
 * @param {!vec3d.Type} translation The vector translation.
 * @param {!vec3d.Type} scale The vector scale.
 * @return {!Type} return mat so that operations can be
 *     chained.
 */
export function makeRotationTranslationScale(mat, rotation, translation, scale) {
  // Quaternion math
  const w = rotation[3];
  const x = rotation[0];
  const y = rotation[1];
 ...
 * @param {!vec3d.Type} translation The vector translation.
 * @param {!vec3d.Type} scale The vector scale.
 * @param {!vec3d.Type} origin The origin about which to scale and
 *     rotate.
 * @return {!Type} return mat so that operations can be
 *     chained.
 */
export function makeRotationTranslationScaleOrigin(mat, rotation, translation, scale, origin) {
  // Quaternion math
  const w = rotation[3];
  const x = rotation[0];
  const y = rotation[1];
 ...
 *
 * @param {!Type} mat The matrix.
 * @param {!vec3d.Type} eyePt The position of the eye point
 *     (camera origin).
 * @param {!vec3d.Type} centerPt The point to aim the camera at.
 * @param {!vec3d.Type} worldUpVec The vector that identifies
...
export function makeLookAt(mat, eyePt, centerPt, worldUpVec) {
  ...
  // normalize.
  const fwdVec = tmpvec4d_[0];
  vec3d.subtract(centerPt, eyePt, fwdVec);
  vec3d.normalize(fwdVec, fwdVec);
  fwdVec[3] = 0;

  // Compute the side vector from the forward vector and the input up vector.
  const sideVec = tmpvec4d_[1];
  vec3d.cross(fwdVec, worldUpVec, sideVec);
  vec3d.normalize(sideVec, sideVec);
  sideVec[3] = 0;

  // Now the up vector to form the orthonormal basis.
  const upVec = tmpvec4d_[2];
  vec3d.cross(sideVec, fwdVec, upVec);
  vec3d.normalize(upVec, upVec);
  upVec[3] = 0;

  // Update the view matrix with the new orthonormal basis and position the
  // camera at the given eye point.
  vec3d.negate(fwdVec, fwdVec);
 ...
 * @param {!vec3d.Type} eyePt The position of the eye point
 *     (camera origin).
 * @param {!vec3d.Type} fwdVec The vector describing where
 *     the camera points to.
 * @param {!vec3d.Type} worldUpVec The vector that
...
export function toLookAt(mat, eyePt, fwdVec, worldUpVec) {
  ...
  if (worldUpVec) {
    ...
    const side = tmpvec3d_[1];
    side[0] = mat[0];
    side[1] = mat[4];
    side[2] = mat[8];
    // Compute up vector as a up = side x forward.
    vec3d.cross(side, fwdVec, worldUpVec);
    // Normalize up vector.
    vec3d.normalize(worldUpVec, worldUpVec);
  }
  return true;
}
 ...
 *
 * @param {!Type} mat The matrix.
 * @param {!vec3d.Type} euler The ZXZ Euler angles in
 *     radians as [theta1, theta2, theta3].
 * @param {boolean=} opt_theta2IsNegative Whether theta2 is in [-pi, 0] instead
 *     of the default [0, pi].
 * @return {!vec4d.Type} return euler so that operations can be
 *     chained together.
 */
 ...
 *
 * @param {!Type} mat The transformation matrix.
 * @param {!vec3d.Type} translation The vector for storing the
 *     result.
 * @return {!vec3d.Type} return translation so that operations can be
 *     chained.
 */
...
/**
 * @type {Array<vec3d.Type>}
 * @private
 */
var tmpvec3d_ = [vec3d.create(), vec3d.create()];


/**
 * @type {Array<vec4d.Type>}
 * @private
 */
var tmpvec4d_ = [vec4d.create(), vec4d.create(), vec4d.create()];
