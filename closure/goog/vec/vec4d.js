 ...
 *
 * @param {!Type} vec The source vec4d.
 * @return {!Type} The new cloned vec4d.
 */
export function clone(vec) {
  const newVec = create();
  setFromVec4d(newVec, vec);
  return newVec;
}
 ...
 *     chained together.
 */
export function setFromVec4d(vec, src) {
  vec[0] = src[0];
  vec[1] = src[1];
  vec[2] = src[2];
  vec[3] = src[3];
 ...
 *     chained together.
 */
export function setFromVec4f(vec, src) {
  vec[0] = src[0];
  vec[1] = src[1];
  vec[2] = src[2];
  vec[3] = src[3];
