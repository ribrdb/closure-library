 ...
 */
export var AUTO_RUN_ONLOAD = goog.define('goog.testing.jsunit.AUTO_RUN_ONLOAD', true);


/**
 * @define {number} Sets a delay in milliseconds after the window onload event
 * and running the tests. See AUTO_RUN_DELAY_IN_MS.
 */
export var AUTO_RUN_DELAY_IN_MS_DEFAULT = goog.define('goog.testing.jsunit.AUTO_RUN_DELAY_IN_MS_DEFAULT', 0);
 ...
 */
export var AUTO_RUN_DELAY_IN_MS = AUTO_RUN_DELAY_IN_MS_DEFAULT;


(function() {
  // Only allow one global test runner to be created on a page.
  if (goog.global['G_testRunner'] instanceof TestRunner) {
    return;
  }
  ...
  window.onerror = function(messageOrEvent, url, line) {
    ...
    if (stack) {
      ...
      if (error.target && error.target.tagName == TagName.SCRIPT) {
  ...
  if (AUTO_RUN_ONLOAD) {
    ...
    window.onload = function(e) {
      ...
      var executionDelayAfterLoad = AUTO_RUN_DELAY_IN_MS;

      realTimeout(function() {
        if (!tr.initialized) {
          var testCase = new TestCase(document.title);
          TestCase.initializeTestCase(testCase);
