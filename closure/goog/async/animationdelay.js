 ...
 * @final
 */
export function AnimationDelay(listener, opt_window, opt_handler) {
 AnimationDelay.base(this, 'constructor');

 /**
   * Identifier of the active delay timeout, or event listener,
   * or null when inactive.
   * @private {?events.Key|number}
   */
 this.id_ = null;
...
goog.inherits(AnimationDelay, Disposable);
...
AnimationDelay.prototype.start = function() {
 ...
 if (raf && !cancelRaf && this.win_.mozRequestAnimationFrame) {
   ...
   this.id_ = this.win_.setTimeout(
       ...
       functions.lock(this.callback_), AnimationDelay.TIMEOUT);
 }
};
 ...
 */
AnimationDelay.prototype.startIfNotActive = function() {
 if (!this.isActive()) {
   this.start();
 }
};
...
AnimationDelay.prototype.disposeInternal = function() {
 this.stop();
 AnimationDelay.base(this, 'disposeInternal');
};
