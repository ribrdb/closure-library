...
goog.declareModuleId('goog.net.browsertestchannel');

import NativeJsonProcessor from '../json/nativejsonprocessor.js';
import { ChannelRequest, ChannelRequest as netChannelRequest } from './channelrequest.js';
 ...
 * @private
 */
BrowserTestChannel.prototype.extraHeaders_ = null;
 ...
 * @private
 */
BrowserTestChannel.prototype.request_ = null;
 ...
 * @private
 */
BrowserTestChannel.prototype.receivedIntermediateResult_ = false;
 ...
 * @private
 */
BrowserTestChannel.prototype.startTime_ = null;
 ...
 * @private
 */
BrowserTestChannel.prototype.firstTime_ = null;
 ...
 * @private
 */
BrowserTestChannel.prototype.lastTime_ = null;
 ...
 * @private
 */
BrowserTestChannel.prototype.path_ = null;
 ...
 * @private
 */
BrowserTestChannel.prototype.state_ = null;
 ...
 * @private
 */
BrowserTestChannel.prototype.lastStatusCode_ = -1;
 ...
 * @private
 */
BrowserTestChannel.prototype.hostPrefix_ = null;
 ...
 * @private
 */
BrowserTestChannel.prototype.blockedPrefix_ = null;
 ...
 * @private
 */
BrowserTestChannel.BLOCKED_TIMEOUT_ = 5000;
 ...
 * @private
 */
BrowserTestChannel.BLOCKED_RETRIES_ = 3;
 ...
 * @private
 */
BrowserTestChannel.BLOCKED_PAUSE_BETWEEN_RETRIES_ = 2000;
 ...
 * @private
 */
BrowserTestChannel.MIN_TIME_EXPECTED_BETWEEN_DATA_ = 500;
 ...
 */
BrowserTestChannel.prototype.setExtraHeaders = function(extraHeaders) {
 ...
 */
BrowserTestChannel.prototype.setParser = function(parser) {
  this.parser_ = parser;
};
...
BrowserTestChannel.prototype.connect = function(path) {
  ...
  if (firstTestResults != null) {
    ...
    if (this.blockedPrefix_) {
      this.state_ = BrowserTestChannel.State_.CHECKING_BLOCKED;
      this.checkBlocked_();
    } else {
      this.state_ = BrowserTestChannel.State_.CONNECTION_TESTING;
      this.connectStage2_();
    }
    return;
  }
  ...
  this.request_ =
      netChannelRequest.createChannelRequest(this, this.channelDebug_);
  this.request_.setExtraHeaders(this.extraHeaders_);
  this.request_.xmlHttpGet(
      sendDataUri, false /* decodeChunks */, null /* hostPrefix */,
      true /* opt_noClose */);
  this.state_ = BrowserTestChannel.State_.INIT;
};
 ...
 * @private
 */
BrowserTestChannel.prototype.checkBlocked_ = function() {
  const uri = this.channel_.createDataUri(
      this.blockedPrefix_, '/mail/images/cleardot.gif');
  uri.makeUnique();
  tmpnetwork.testLoadImageWithRetries(
      uri.toString(), BrowserTestChannel.BLOCKED_TIMEOUT_,
      goog.bind(this.checkBlockedCallback_, this),
      BrowserTestChannel.BLOCKED_RETRIES_,
      BrowserTestChannel.BLOCKED_PAUSE_BETWEEN_RETRIES_);
  this.notifyServerReachabilityEvent(
      ServerReachability.REQUEST_MADE);
};
 ...
 * @private
 */
BrowserTestChannel.prototype.checkBlockedCallback_ = function(
    succeeded) {
  if (succeeded) {
    this.state_ = BrowserTestChannel.State_.CONNECTION_TESTING;
    this.connectStage2_();
  ...
  if (succeeded) {
    this.notifyServerReachabilityEvent(
        ServerReachability.REQUEST_SUCCEEDED);
  }
};
 ...
 * @private
 */
BrowserTestChannel.prototype.connectStage2_ = function() {
  ...
  if (secondTestResults != null) {
    this.channelDebug_.debug(
        'TestConnection: skipping stage 2, precomputed result is ' +
            ...
            'Unbuffered');
    stats.notifyStatEvent(
        stats.Stat.TEST_STAGE_TWO_START);
  ...
  stats.notifyStatEvent(
      stats.Stat.TEST_STAGE_TWO_START);
  if (!netChannelRequest.supportsXhrStreaming()) {
 ...
 * @return {!XhrIo} New XhrIo object.
 */
BrowserTestChannel.prototype.createXhrIo = function(hostPrefix) {
 ...
 */
BrowserTestChannel.prototype.abort = function() {
  if (this.request_) {
 ...
 */
BrowserTestChannel.prototype.isClosed = function() {
  return false;
};
 ...
 */
BrowserTestChannel.prototype.onRequestData = function(
    req, responseText) {
  this.lastStatusCode_ = req.getLastStatusCode();
  if (this.state_ == BrowserTestChannel.State_.INIT) {
    ...
    if (!responseText) {
      ...
      this.channel_.testConnectionFailure(
          this, netChannelRequest.Error.BAD_DATA);
      return;
    }
    ...
    if (this.receivedIntermediateResult_) {
      stats.notifyStatEvent(
          stats.Stat.TEST_STAGE_TWO_DATA_TWO);
      ...
      if (responseText == '11111') {
        stats.notifyStatEvent(
            stats.Stat.TEST_STAGE_TWO_DATA_ONE);
 ...
 * @param {ChannelRequest} req  The request object.
 */
BrowserTestChannel.prototype.onRequestComplete = function(req) {
  ...
  if (!this.request_.getSuccess()) {
    ...
    if (this.state_ == BrowserTestChannel.State_.INIT) {
      stats.notifyStatEvent(
          stats.Stat.TEST_STAGE_ONE_FAILED);
    } else if (
        this.state_ == BrowserTestChannel.State_.CONNECTION_TESTING) {
      stats.notifyStatEvent(
          stats.Stat.TEST_STAGE_TWO_FAILED);
    }
    this.channel_.testConnectionFailure(
        this,
 ...
 */
BrowserTestChannel.prototype.getLastStatusCode = function() {
 ...
 */
BrowserTestChannel.prototype.shouldUseSecondaryDomains = function() {
 ...
 * @private
 */
BrowserTestChannel.prototype.checkForEarlyNonBuffered_ = function() {
  /** @suppress {strictPrimitiveOperators} */
  ...
  return netChannelRequest.supportsXhrStreaming() ||
      ms < BrowserTestChannel.MIN_TIME_EXPECTED_BETWEEN_DATA_;
};
 ...
 */
BrowserTestChannel.prototype.notifyServerReachabilityEvent = function(
    reachabilityType) {
