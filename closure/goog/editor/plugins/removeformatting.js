...
goog.inherits(RemoveFormatting, Plugin);
 ...
 * @type {string}
 */
RemoveFormatting.REMOVE_FORMATTING_COMMAND =
    '+removeFormat';
 ...
 * @private
 */
RemoveFormatting.appendNewline_ = function(sb) {
  sb.push('<br>');
};
...
/** @override */
RemoveFormatting.prototype.getTrogClassId = function() {
  return 'RemoveFormatting';
};


/** @override */
RemoveFormatting.prototype.isSupportedCommand = function(
    command) {
  return command ==
      RemoveFormatting.REMOVE_FORMATTING_COMMAND;
};
...
/** @override */
RemoveFormatting.prototype.execCommandInternal = function(
    command, var_args) {
  if (command ==
      RemoveFormatting.REMOVE_FORMATTING_COMMAND) {
...
/** @override */
RemoveFormatting.prototype.handleKeyboardShortcut =
    function(e, key, isModifierPressed) {
      ...
      if ((!(userAgent.MAC || platform.isChromeOS()) &&
           key === ' ') ||
          (key === '\\')) {
 ...
 * @private
 */
RemoveFormatting.prototype.removeFormatting_ = function() {
 ...
 */
RemoveFormatting.prototype.getTableAncestor_ = function(
    nodeToCheck) {
  var fieldElement = this.getFieldObject().getElement();
  while (nodeToCheck && nodeToCheck != fieldElement) {
    if (nodeToCheck.tagName == TagName.TABLE) {
      return nodeToCheck;
    }
 ...
 * move it into editorRange. The Firefox implementation uses execCommand
 ...
 * @suppress {strictMissingProperties} Added to tighten compiler checks
 */
RemoveFormatting.prototype.pasteHtml_ = function(html) {
  ...
  if (parent.tagName == TagName.A) {
    ...
    safe.insertAdjacentHtml(
        parent, safe.InsertAdjacentHtmlPosition.AFTERBEGIN,
        safePlaceholderAnchorContent);
    safe.insertAdjacentHtml(
        parent, safe.InsertAdjacentHtmlPosition.BEFOREEND,
        safePlaceholderAnchorContent);
  }
  ...
  if (userAgent.GECKO) {
    ...
    while ((parent = dummySpan.parentNode) &&
           editorNode.isEmpty(parent) &&
           !editorNode.isEditableContainer(parent)) {
      var tag = parent.nodeName;
      // We can't remove these table tags as it will invalidate the table dom.
      if (tag == TagName.TD || tag == TagName.TR ||
          tag == TagName.TH) {
        break;
      }
      ...
      dom.insertSiblingAfter(dummySpan, parent);
  ...
  Range
      ...
      .select();
  dom.removeNode(startSpan);
  dom.removeNode(endSpan);
};
 ...
 * editorRange.  The main reason it can't be moved is because we need to
 ...
 * @suppress {strictMissingProperties} Added to tighten compiler checks
 */
RemoveFormatting.prototype.getHtmlText_ = function(range) {
  var div = this.getFieldDomHelper().createDom(TagName.DIV);
 ...
 * @private
 */
RemoveFormatting.prototype.adjustRangeForTables_ = function(
    range, startInTable, endInTable) {
  // Create placeholders for the current selection so we can restore it
  // later.
  var savedCaretRange = editorRange.saveUsingNormalizedCarets(range);
  ...
  if (startInTable) {
    ...
    dom.insertSiblingAfter(textNode, startInTable);
    startNode = textNode;
    startOffset = 0;
  }
  ...
  if (endInTable) {
    var textNode = dh.createTextNode('');
    dom.insertSiblingBefore(textNode, endInTable);
    endNode = textNode;
    endOffset = 0;
  }

  Range.createFromNodes(startNode, startOffset, endNode, endOffset)
      .select();
 ...
 * @private
 */
RemoveFormatting.prototype.putCaretInCave_ = function(
    caretRange, isStart) {
  var cavedCaret = dom.removeNode(caretRange.getCaret(isStart));
  if (isStart) {
    /** @suppress {strictMissingProperties} Added to tighten compiler checks */
 ...
 * @suppress {strictMissingProperties} Added to tighten compiler checks
 */
RemoveFormatting.prototype.restoreCaretsFromCave_ =
    function() {
 ...
 * @private
 */
RemoveFormatting.prototype.convertSelectedHtmlText_ =
    function(convertFunc) {
      ...
      if (userAgent.GECKO || userAgent.EDGE) {
        ...
        var expandedRange =
            editorRange.expand(range, this.getFieldObject().getElement());
      ...
      if ((userAgent.GECKO || userAgent.EDGE) && savedCaretRange) {
        ...
        var restoredRange =
            RemoveFormatting.createRangeDelimitedByRanges_(
                startRange, endRange);
 ...
 * @private
 */
RemoveFormatting.prototype.removeFormattingWorker_ =
    function(html) {
      var el = dom.createElement(TagName.DIV);
      safe.setInnerHtml(
          el, legacyconversions.safeHtmlFromString(html));
      ...
      for (var sp = 0; sp >= 0; sp -= 2) {
        ...
        if (changedLevel) {
          RemoveFormatting.appendNewline_(sb);
        }
        ...
        while (numNodesProcessed < nodeList.length) {
          ...
          switch (nodeName) {
            ...
            case String(TagName.TABLE):
              RemoveFormatting.appendNewline_(sb);
              tableStack[tableLevel++] = sp;
              break;

            case String(TagName.PRE):
            case 'XMP':
            ...
            case String(TagName.SELECT):
              continue;

            case String(TagName.A):
            ...
            case String(TagName.DIV):
              var parent = node.parentNode;
              if (parent.firstChild == node &&
                  RemoveFormatting.BLOCK_RE_.test(
                      parent.tagName)) {
            ...
            default:
              if (RemoveFormatting.BLOCK_RE_.test(nodeName)) {
                RemoveFormatting.appendNewline_(sb);
              }
          }
 ...
 */
RemoveFormatting.prototype.getValueForNode = function(
    node) {
  return null;
};
 ...
 *     meet your needs.
 */
RemoveFormatting.prototype.setRemoveFormattingFunc =
