 ...
 * This file provides the class Table and two
 * supporting classes, TableRow and
 * TableCell. Together these provide support for
...
export function Table(node) {
  ...
  this.element =
      googDom.getAncestorByTagNameAndClass(node, TagName.TABLE);
  if (!this.element) {
    log.error(
        this.logger_, "Can't create Table based on a node " +
            "that isn't a table, or descended from a table.");
  }
  this.dom_ = googDom.getDomHelper(this.element);
  this.refresh();
}
 ...
 * @private
 */
Table.prototype.logger_ = log.getLogger('goog.editor.Table');
...
Table.prototype.refresh = function() {
  var rows = this.rows = [];
  var tbody = googDom.getElementsByTagName(
      TagName.TBODY, asserts.assert(this.element))[0];
  if (!tbody) {
    return;
  }
  var trs = [];
  for (var child = tbody.firstChild; child; child = child.nextSibling) {
    if (child.nodeName == TagName.TR) {
      trs.push(child);
    }
  }
  ...
  for (var rowNum = 0, tr; tr = trs[rowNum]; rowNum++) {
    ...
    var tds = Table.getChildCellElements(tr);
    var columnNum = 0;
    ...
    for (var cellNum = 0, td; td = tds[cellNum]; cellNum++) {
      ...
      var cell = new TableCell(td, rowNum, columnNum);
      ...
      for (var i = 0; i < cell.rowSpan; i++) {
        ...
        if (!cellRow) {
          ...
          rows.push(
              cellRow = new TableRow(trs[cellRowNum], cellRowNum));
        }
 ...
 */
Table.getChildCellElements = function(tr) {
  var cells = [];
  for (var i = 0, cell; cell = tr.childNodes[i]; i++) {
    if (cell.nodeName == TagName.TD ||
        cell.nodeName == TagName.TH) {
      cells.push(cell);
    }
  }
  return cells;
 ...
 */
Table.prototype.insertRow = function(opt_rowIndex) {
  ...
  var newTr = this.dom_.createElement(TagName.TR);
  for (var i = 0, cell; cell = refRow.columns[i]; i += 1) {
    ...
    if (insertAfter) {
      googDom.insertSiblingAfter(newTr, refRow.element);
    } else {
      googDom.insertSiblingBefore(newTr, refRow.element);
    }
  }
  this.refresh();
  return newTr;
 ...
 */
Table.prototype.insertColumn = function(opt_colIndex) {
  ...
  for (var rowNum = 0, row; row = this.rows[rowNum]; rowNum++) {
    ...
    if (existingCell && existingCell.endCol >= colIndex &&
      ...
      newTd.style.width = Table.OPTIMUM_EMPTY_CELL_WIDTH + 'px';
 ...
 */
Table.prototype.removeRow = function(rowIndex) {
 ...
 */
Table.prototype.removeColumn = function(colIndex) {
...
Table.prototype.mergeCells = function(
  ...
  if (startRowIndex == endRowIndex && startColIndex == endColIndex) {
    log.warning(this.logger_, "Can't merge single cell");
    return false;
  }
  ...
  for (var i = 1; cell = cells[i]; i++) {
    ...
    if (targetTd.lastChild &&
        targetTd.lastChild.nodeType == NodeType.TEXT) {
...
Table.prototype.splitCell = function(rowIndex, colIndex) {
  var row = this.rows[rowIndex];
  var cell = row.columns[colIndex];
  var newTds = [];
  for (var i = 0; i < cell.rowSpan; i++) {
 ...
 */
Table.prototype.insertCellElement = function(
 ...
 */
Table.prototype.createEmptyTd = function() {
  // TODO(user): more cross-browser testing to determine best
  // and least annoying filler content.
  return this.dom_.createDom(TagName.TD, {}, Unicode.NBSP);
};
 ...
 * @type {number}
 */
Table.OPTIMUM_EMPTY_CELL_WIDTH = 60;
 ...
 * @type {number}
 */
Table.OPTIMUM_MAX_NEW_TABLE_WIDTH = 600;
 ...
 * @type {string}
 */
Table.DEFAULT_BORDER_COLOR = '#888';
...
Table.createDomTable = function(
  ...
  var cellWidth = Math.max(
      ...
      Math.min(
          ...
          Table.OPTIMUM_MAX_NEW_TABLE_WIDTH / columns));

  var tds = googDom.getElementsByTagName(TagName.TD, tableElement);
  ...
  googStyle.setStyle(tableElement, {
