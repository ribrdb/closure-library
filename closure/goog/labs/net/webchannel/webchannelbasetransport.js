 ...
 * @final
 */
export function WebChannelBaseTransport() {
...
goog.scope(function() {
  ...
  WebChannelBaseTransport.Channel = function(url, opt_options) {
    ...
    this.channel_ = new WebChannelBase(
        opt_options, WebChannelTransport.CLIENT_VERSION);
    ...
    if (opt_options && opt_options.messageContentType) {
      if (initHeaders) {
        ...
        initHeaders = object.create(
            WebChannel.X_WEBCHANNEL_CONTENT_TYPE,
            opt_options.messageContentType);
      }
    }
    ...
    if (opt_options && opt_options.clientProfile) {
      if (initHeaders) {
        ...
        initHeaders = object.create(
            WebChannel.X_WEBCHANNEL_CLIENT_PROFILE,
            opt_options.clientProfile);
      }
    }
  ...
  goog.inherits(WebChannelBaseTransport.Channel, EventTarget);
  ...
  WebChannelBaseTransport.Channel.prototype.messageToMapObject_ = function(
    ...
    if (this.sendRawJson_) {
      const rawJson = {};
      rawJson[Wire.RAW_DATA_KEY] = json.serialize(message);
      return rawJson;
    }
  ...
  WebChannelBaseTransport.Channel.MessageEvent = function(array) {
    ...
    if (metadata) {
      this.metadataKey = object.getAnyKey(metadata);
      if (this.metadataKey) {
        this.data = object.get(metadata, this.metadataKey);
      } else {
