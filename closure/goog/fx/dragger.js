...
goog.declareModuleId('goog.fx.dragger');
 ...
 * @struct
 */
export function Dragger(target, opt_handle, opt_limits) {
 Dragger.base(this, 'constructor');
...
goog.inherits(Dragger, EventTarget);
 ...
 * @private
 */
Dragger.HAS_SET_CAPTURE_ = goog.global.document &&
 ...
 */
Dragger.cloneNode = function(sourceEl) {
 ...
 */
Dragger.prototype.setAllowSetCapture = function(allow) {
 ...
 */
Dragger.prototype.enableRightPositioningForRtl = function(
 ...
 *     prevented.
 */
Dragger.prototype.setPreventMouseDown = function(preventMouseDown) {
...
Dragger.prototype.setupDragHandlers = function() {
 ...
 this.eventHandler_.listen(
     doc, [EventType.TOUCHMOVE, EventType.MOUSEMOVE],
     this.handleMove_, {capture: useCapture, passive: false});
 this.eventHandler_.listen(
     doc, [EventType.TOUCHEND, EventType.MOUSEUP],
     this.endDrag, useCapture);

 if (this.useSetCapture_) {
   docEl.setCapture(false);
   this.eventHandler_.listen(
       docEl, EventType.LOSECAPTURE, this.endDrag);
 } else {
   ...
   this.eventHandler_.listen(
       dom.getWindow(doc), EventType.BLUR, this.endDrag);
 }

 if (userAgent.IE && this.ieDragStartCancellingOn_) {
   // Cancel IE's 'ondragstart' event.
   this.eventHandler_.listen(
       doc, EventType.DRAGSTART, Event.preventDefault);
 }

 if (this.scrollTarget_) {
   this.eventHandler_.listen(
       this.scrollTarget_, EventType.SCROLL, this.onScroll_,
       useCapture);
 }
};
...
Dragger.prototype.fireDragStart_ = function(e) {
 return this.dispatchEvent(new DragEvent(
     Dragger.EventType.START, this, e.clientX, e.clientY, e));
};
...
Dragger.prototype.endDrag = function(e, opt_dragCanceled) {
 ...
 if (this.dragging_) {
   ...
   var dragCanceled =
       opt_dragCanceled || e.type == EventType.TOUCHCANCEL;
   this.dispatchEvent(
       new DragEvent(
           Dragger.EventType.END, this, e.clientX, e.clientY, e, x, y,
           dragCanceled));
 } else {
   this.dispatchEvent(Dragger.EventType.EARLY_CANCEL);
 }
 ...
 if (this.enabled_) {
   ...
   if (this.dragging_) {
     var rv = this.dispatchEvent(
         new DragEvent(
             Dragger.EventType.BEFOREDRAG, this, e.clientX, e.clientY,
             e, x, y));
...
Dragger.prototype.doDrag = function(e, x, y, dragFromScroll) {
 ...
 this.dispatchEvent(
     new DragEvent(
         Dragger.EventType.DRAG, this, e.clientX, e.clientY, e, x, y));
};
 ...
 * @protected
 */
Dragger.prototype.computeInitialPosition = function() {
